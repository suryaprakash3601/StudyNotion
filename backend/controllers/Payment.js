...
exports.verifySignature = async (req, res) => {
  try {
    ...
    // signature matched — enroll the student
    ...
    return res.status(500).json({
      message: error.message || "Error while verifying signature",
      success: false,
    });
  }
};
...
const enrollStudents = async (userId, courses) => {
  ...
  for (const courseId of courses) {
    try {
      ...
      // Create a course progress document for the student
      ...
      // Find the student, add the course and progress reference
      ...
      // Send enrollment confirmation email (non-fatal if SMTP fails)
      try {
        await mailSender(
        ...
        console.warn("SMTP failure, could not send enrollment confirmation email:", mailErr.message);
      }
    } catch (error) {
      console.error("Enrollment error for course", courseId, error.message);
...
exports.sendPaymentSuccessEmail = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("SMTP failure, could not send payment success email:", mailErr.message);
    }
    return res.status(200).json({ success: true, message: "Payment confirmation recorded" });
  } catch (error) {
    console.log("error in recording payment confirmation", error);
    return res
      .status(500)
      .json({ success: false, message: "Could not record payment success" });
  }
};
